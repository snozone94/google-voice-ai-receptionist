...
- [ ] Upgrade the Render service from Free to Starter so the persistent disk can attach.
- [ ] Confirm `DATA_DIR=/var/data/ddd-ai-dispatch` is set on Render.
- [ ] Confirm `/api/setup-status` shows `persistentStorage: true`.
